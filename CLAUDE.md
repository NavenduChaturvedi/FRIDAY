...
- **`Brain` builds its provider list at construction.** A provider that
  can't initialise (no API key, Ollama down) is skipped with a stderr note;
  if none survive, `BrainError` is raised before the loop starts. At ask
  time, providers are tried in order and the first non-empty reply is what
